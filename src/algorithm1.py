...
def find_duplicates_naive(arr):
    """
    Naive duplicate detection using nested loops
    Time Complexity: O(n²)
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if arr[i] == arr[j] and arr[i] not in seen_duplicates:
                duplicates.append(arr[i])
                seen_duplicates.add(arr[i])
...
def test_naive_algorithm():
    """Test naive algorithm with 3 sizes: 10, 100, 1000"""
    ...
    for size in sizes:
        ...
        duplicates = find_duplicates_naive(test_data)
        end_time = time.time()
